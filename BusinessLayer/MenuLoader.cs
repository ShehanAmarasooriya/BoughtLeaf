...
namespace BoughtLeafBusinessLayer
{
    public class MenuLoader
    {
        ...
        {
            DataSet ds = SQLHelper.FillDataSet("SELECT Menu.MenuItem, Menu.MenuName FROM UserPermission INNER JOIN Menu ON UserPermission.MenuItemID = Menu.MenuItemID WHERE (UserPermission.UserID = '" + BoughtLeafBusinessLayer.BLUser.StrUserName+ "') ORDER BY Menu.MenuName", CommandType.Text);
            return ds;
        }

        public DataSet getRoleMenus()
        {
            DataSet roleDs = SQLHelper.FillDataSet("SELECT top 1 UserRole FROM dbo.[User] WHERE (UserID = '" + BoughtLeafBusinessLayer.BLUser.StrUserName+ "')", CommandType.Text);
        ...
        {
            SQLHelper.ExecuteNonQuery("INSERT INTO [dbo].[Menu] ( [MenuItem] ,[MenuName],MenuID) VALUES ('" + SubMenu + "' ,'" + MainMenu + "','" + SubMenu + "')", CommandType.Text);
        }
        ...
        public DataTable ListAllMenuItems()
        {
            ...
            dtrow = dt.NewRow();
            dataReader = SQLHelper.ExecuteReader("SELECT     TOP (100) PERCENT MenuName, MenuItem FROM dbo.Menu order by MenuName, MenuItem", CommandType.Text);
